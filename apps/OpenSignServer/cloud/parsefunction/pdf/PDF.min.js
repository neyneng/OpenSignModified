...
async function PDF(i) {
    try {
        ...
        console.log('Document Data:', o.data);
        console.log('User Data:', t.data);
        if (!t.data || !t.data.objectId) {
            console.log('User not allowed:', t.data);
        ...
        }
        {
            ...
            console.log('Contact Data:', s.data.results[0]);
            console.log('User Type:', r);
            if (!i.params.pdfFile) {
                console.log('PDF file not present:', i.params.pdfFile);
            ...
            }
            {
                ...
                console.log('PDF Buffer:', e);
                console.log('PFX Base64:', g);
                console.log('Signing User:', u);
                let a;
                var f = (a =
                    o.data.AuditTrail && 0 < o.data.AuditTrail.length
                        ? [...o.data.AuditTrail, u]
                        : [u]).filter(e => 'Signed' === e.Activity);
                console.log('Audit Trail:', f);
                let t = !1;
                !(
                    (o.data.Signers && 0 < o.data.Signers.length && f.length !== o.data.Signers.length) ||
                    !(t = !0)
                );
                console.log('Check Signers:', t);
                var y, P, v, b, x, w, U, I, S;
                var A = `exported_file_${Math.floor(5e3 * Math.random())}.pdf`;
                var D = './exports/' + A;
                var E;

                if (t) {
                    y = o.data.Signers?.map(e => e.Name + ' <' + e.Email + '>');
                    console.log('Signer Names:', y);
                    if (y && y.length > 0) {
                        P = await PDFDocument.load(e);
                        v = await pdflibAddPlaceholder({
                            pdfDoc: P,
                            reason: 'Digitally signed by OpenSign for ' + y.join(', '),
                            location: 'location',
                            signatureLength: 15000
                        });
                        E = Buffer.from(await P.save());
                    } else {
                        b = await PDFDocument.load(e);
                        x = await pdflibAddPlaceholder({
                            pdfDoc: b,
                            reason: 'Digitally signed by OpenSign for ' + p + ' <' + m + '>',
                            location: 'location',
                            signatureLength: 15000
                        });
                        E = Buffer.from(await b.save());
                    }

                    w = await new SignPDF(E, h).signPDF();
                    // Log if signature is successfully attached
                    console.log('Signature attached:', w);
